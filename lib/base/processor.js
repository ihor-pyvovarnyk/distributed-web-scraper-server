...
class TemplateEngine {
    ...
    _compileFind(findValue, domContext=null) {
        ...
        let innerContext = selector
            ? domContext ? domContext.find(selector) : this.$(selector)
    ...
    _compileGet(getValue, domContext) {
        ...
        if (typeof getValue === 'string') {
            switch (getValue) {
                case 'text': value = domContext.eq(0).text(); break;
                case 'int': value = parseInt(domContext.eq(0).text(), 10); break;
                case 'float': value = parseFloat(domContext.eq(0).text()); break;
                case 'html': value = domContext.eq(0).html(); break;
